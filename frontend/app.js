...
        {"고유ID":122,"가게이름":"우리과일","가게분류":"청과물","main_product":"수박, 참외, 포도, 복숭아, 자두, 계절과일","위치":"서울 동대문구","특징":"제철을 맞은 신선하고 당도 높은 과일을 전문적으로 판매합니다.","좌표":"744,470","url":"singular-kheer-0305fe.netlify.app"},
    ...
    const mealKitData = [
        {
            ...
            ingredients: [
                { name: '잘 익은 김치 300g', request: '찌개용' }, { name: '돼지고기 목살 200g', request: '찌개용' },
                { name: '두부 1/2모', request: '' }, { name: '대파 1/2대', request: '어슷썰기' },
                { name: '양파 1/4개', request: '' }, { name: '고춧가루 1큰술', request: '' }
            ]
        },
        {
            id: 'mk2', name: '소고기 된장찌개', emoji: '🍲', description: '구수한 된장과 부드러운 소고기의 조화',
            ingredients: [
                { name: '소고기 양지 150g', request: '찌개용' }, { name: '된장 2큰술', request: '' },
                { name: '두부 1/2모', request: '' }, { name: '애호박 1/3개', request: '' },
                { name: '양파 1/4개', request: '' }, { name: '청양고추 1개', request: '' }
            ]
        },
        {
            id: 'mk3', name: '매콤 제육볶음', emoji: '🍳', description: '매콤달콤 밥도둑 대표 메뉴',
            ingredients: [
                { name: '돼지고기 앞다리살 400g', request: '제육용' }, { name: '양파 1/2개', request: '' },
                { name: '대파 1대', request: '' }, { name: '깻잎 10장', request: '' },
                { name: '고추장 3큰술', request: '' }, { name: '다진 마늘 1큰술', request: '' }
            ]
        }
    ];
    ...
    let shoppingCart = {}, currentCategorySort = 'default', completedOrders = [], favoriteStores = [],
        userRecipes = [], recentlyViewedStores = [], notifications = [];
    ...
    function updateCartCountIndicator() { const countElement = document.getElementById('cart-item-count'); let totalCount = 0; for (const storeId in shoppingCart) { for (const cartItemId in shoppingCart[storeId].items) { totalCount += shoppingCart[storeId].items[cartItemId].quantity; } } countElement.textContent = totalCount; countElement.classList.toggle('hidden', totalCount === 0); }
    function saveRecipesToStorage() { localStorage.setItem('cheongnyamri.recipes', JSON.stringify(userRecipes)); }
    // ==================== 화면 전환 최종 함수 ====================
    function showScreen(screenId, param = null) {
        ...
        if (screenId === 'mapScreen') { appContainer.classList.remove('max-w-lg'); } else { appContainer.classList.add('max-w-lg'); }
    ...
    function showToast(message) { const container = document.getElementById('toast-container'); const toast = document.createElement('div'); toast.className = 'toast'; toast.textContent = message; container.appendChild(toast); setTimeout(() => { toast.remove(); }, 3000); }
    function loadNotificationsFromStorage() { const storedNotifications = localStorage.getItem('cheongnyamri.notifications'); notifications = storedNotifications ? JSON.parse(storedNotifications) : []; updateNotificationIndicator(); }
    function saveNotificationsToStorage() { localStorage.setItem('cheongnyamri.notifications', JSON.stringify(notifications)); }
    function updateNotificationIndicator() { const dot = document.getElementById('notification-dot'); const hasUnread = notifications.some(n => !n.read); dot.classList.toggle('hidden', !hasUnread); }
    function showNotificationModal() { const modal = document.createElement('div'); modal.className = 'modal-overlay'; let notificationsHtml = notifications.length === 0 ? `<p class="text-center text-gray-500 py-10">새로운 알림이 없습니다.</p>` : notifications.map(n => `<div class="p-3 border-b last:border-b-0 ${!n.read ? 'bg-indigo-50' : ''}"><p class="text-sm text-gray-800">${n.message}</p></div>`).join(''); modal.innerHTML = `<div class="modal-content flex flex-col h-full max-h-[80vh]"><div class="flex justify-between items-center mb-4"><h3 class="text-xl font-bold">알림</h3><button onclick="this.closest('.modal-overlay').remove()"><i class="ph ph-x text-2xl"></i></button></div><div class="flex-1 overflow-y-auto -mx-6 px-6">${notificationsHtml}</div></div>`; document.getElementById('modalContainer').appendChild(modal); notifications.forEach(n => n.read = true); saveNotificationsToStorage(); updateNotificationIndicator(); }
    function simpleHash(str) { let hash = 0; for (let i = 0; i < str.length; i++) { const char = str.charCodeAt(i); hash = (hash << 5) - hash + char; hash &= hash; } return hash; }
    function showAddToCartModal(productData) { const modal = document.createElement('div'); modal.className = 'modal-overlay'; modal.innerHTML = `<div class="modal-content"><h3 class="text-xl font-bold mb-2">${productData.productName}</h3><p class="text-gray-600 mb-4">상품을 장바구니에 담습니다.</p><input type="text" id="itemRequestInput" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 mb-4" placeholder="특별 요청사항 (예: 찌개용으로 썰어주세요)"><div class="flex flex-col space-y-2"><button id="confirmAddToCartBtn" class="w-full px-4 py-3 bg-indigo-600 text-white rounded-lg font-semibold hover:bg-indigo-700 transition-colors">장바구니 담기</button><button onclick="this.closest('.modal-overlay').remove()" class="w-full px-4 py-2 border rounded-lg text-gray-700 hover:bg-gray-100 transition-colors">취소</button></div></div>`; document.getElementById('modalContainer').appendChild(modal); document.getElementById('confirmAddToCartBtn').addEventListener('click', () => { const request = document.getElementById('itemRequestInput').value.trim(); addToCart(productData.storeId, productData.storeName, productData.productId, productData.productName, parseInt(productData.productPrice, 10), productData.productEmoji, request); modal.remove(); showToast(`'${productData.productName}' 상품을 장바구니에 담았습니다!`); }); }
    function addToCart(storeId, storeName, productId, productName, price, emoji, request = "") { if (!shoppingCart[storeId]) { shoppingCart[storeId] = { storeName: storeName, items: {} }; } const cartItemId = `${productId}-${simpleHash(request)}`; if (shoppingCart[storeId].items[cartItemId]) { shoppingCart[storeId].items[cartItemId].quantity++; } else { shoppingCart[storeId].items[cartItemId] = { productId, name: productName, price, quantity: 1, emoji, request }; } updateCartCountIndicator(); }
    function updateCartItemQuantity(storeId, cartItemId, change) { if (shoppingCart[storeId] && shoppingCart[storeId].items[cartItemId]) { const item = shoppingCart[storeId].items[cartItemId]; item.quantity += change; if (item.quantity <= 0) { delete shoppingCart[storeId].items[cartItemId]; if (Object.keys(shoppingCart[storeId].items).length === 0) { delete shoppingCart[storeId]; } } } updateCartCountIndicator(); const existingModal = document.querySelector('.modal-overlay'); if (existingModal) existingModal.remove(); showCartModal(); }
    function showCartModal() { const modal = document.createElement('div'); modal.className = 'modal-overlay'; let cartItemsHtml = ''; let totalPrice = 0; const cartIsEmpty = Object.keys(shoppingCart).length === 0; if (cartIsEmpty) { cartItemsHtml = `<p class="text-center text-gray-500 py-10">장바구니가 비어 있습니다.</p>`; } else { for (const storeId in shoppingCart) { const store = shoppingCart[storeId]; let storeItemsHtml = ''; for (const cartItemId in store.items) { const item = store.items[cartItemId]; totalPrice += item.price * item.quantity; const requestHtml = item.request ? `<p class="text-xs text-indigo-600 bg-indigo-50 rounded px-2 py-1 mt-1">요청: ${item.request}</p>` : ''; storeItemsHtml += `<div class="flex items-start py-3 border-b last:border-b-0"><div class="w-16 h-16 rounded-md mr-4 bg-gray-200 flex items-center justify-center text-3xl">${item.emoji}</div><div class="flex-1"><p class="font-semibold text-gray-800">${item.name}</p><p class="text-gray-600 font-bold">${item.price.toLocaleString()}원</p>${requestHtml}</div><div class="flex items-center space-x-2"><button class="cart-quantity-btn" data-store-id="${storeId}" data-cart-item-id="${cartItemId}" data-change="-1">-</button><span class="w-8 text-center font-semibold">${item.quantity}</span><button class="cart-quantity-btn" data-store-id="${storeId}" data-cart-item-id="${cartItemId}" data-change="1">+</button></div><button class="ml-4 text-gray-400 hover:text-red-500 cart-delete-btn" data-store-id="${storeId}" data-cart-item-id="${cartItemId}"><i class="ph ph-x-circle text-2xl"></i></button></div>`; } cartItemsHtml += `<div class="mb-4"><button class="flex items-center font-bold text-lg mb-2 store-link-from-cart" data-store-id="${storeId}">${store.storeName} <i class="ph ph-caret-right ml-1"></i></button><div class="bg-gray-50 p-2 rounded-lg">${storeItemsHtml}</div></div>`; } } modal.innerHTML = `<div class="modal-content flex flex-col h-full max-h-[80vh]"><div class="flex justify-between items-center mb-4"><h3 class="text-xl font-bold">장바구니</h3><button onclick="this.closest('.modal-overlay').remove()"><i class="ph ph-x text-2xl"></i></button></div><div class="flex-1 overflow-y-auto pr-2">${cartItemsHtml}</div><div class="border-t pt-4 mt-4"><div class="mb-4"><label for="userIdInput" class="block text-sm font-medium text-gray-700 mb-1">사용자 ID</label><input type="text" id="userIdInput" placeholder="사용자 ID를 입력해주세요" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500"></div><div class="text-right font-bold text-xl mb-4">총 결제금액: <span class="text-indigo-600">${totalPrice.toLocaleString()}원</span></div><div class="flex flex-col space-y-2"><button id="checkoutBtn" class="w-full px-4 py-3 bg-indigo-600 text-white rounded-lg font-semibold hover:bg-indigo-700 transition-colors ${cartIsEmpty ? 'opacity-50 cursor-not-allowed' : ''}" ${cartIsEmpty ? 'disabled' : ''}>결제하고 픽업하기</button></div></div></div>`; document.getElementById('modalContainer').appendChild(modal); modal.addEventListener('click', e => { const quantityBtn = e.target.closest('.cart-quantity-btn'); const deleteBtn = e.target.closest('.cart-delete-btn'); const storeLink = e.target.closest('.store-link-from-cart'); if (quantityBtn) { const { storeId, cartItemId, change } = quantityBtn.dataset; updateCartItemQuantity(storeId, cartItemId, parseInt(change)); } else if (deleteBtn) { const { storeId, cartItemId } = deleteBtn.dataset; updateCartItemQuantity(storeId, cartItemId, -Infinity); } else if (storeLink) { const { storeId } = storeLink.dataset; modal.remove(); showScreen('storeDetailScreen', { storeId: parseInt(storeId, 10), from: 'homeScreen' }); } }); if (!cartIsEmpty) { document.getElementById('checkoutBtn').addEventListener('click', handleCheckout); } }
    function renderCategoryShortcuts() { const container = document.getElementById('category-shortcut-container'); container.innerHTML = Object.entries(categoryMap).map(([name, { emoji, color }]) => `<button onclick="showScreen('categoryListScreen', '${name}')" class="flex flex-col items-center space-y-2"><div class="w-full h-16 flex items-center justify-center bg-${color}-100 rounded-lg shadow-sm text-4xl">${emoji}</div><span class="text-xs font-medium">${name}</span></button>`).join(''); }
    function renderCategoryList(categoryKey) { const container = document.getElementById('categoryStoreListContainer'); let stores; if (categoryKey === 'all') { document.getElementById('categoryTitle').textContent = '전체 가게'; stores = [...storeData]; } else { const categoryInfo = categoryMap[categoryKey]; if (!categoryInfo) return; document.getElementById('categoryTitle').textContent = categoryKey; stores = storeData.filter(s => categoryInfo.keywords.includes(s.category)); } container.innerHTML = ''; switch (currentCategorySort) { case 'orders': stores.sort((a, b) => b.orders - a.orders); break; case 'rating': stores.sort((a, b) => b.rating - a.rating); break; } if (stores.length === 0) { container.innerHTML = `<p class="text-center text-gray-500 py-10">해당 카테고리의 가게가 없습니다.</p>`; return; } const uniqueStores = Array.from(new Map(stores.map(store => [store.id, store])).values()); uniqueStores.forEach(store => { const tagsHtml = store.mainTags.map(tag => `<span class="bg-gray-200 text-gray-700 text-xs font-medium mr-2 px-2 py-1 rounded">${tag}</span>`).join(''); const card = document.createElement('div'); card.className = 'flex items-start bg-white p-4 rounded-lg border border-gray-200 relative cursor-pointer store-link'; card.dataset.storeId = store.id; card.dataset.from = 'categoryListScreen'; card.dataset.category = categoryKey; card.innerHTML = `<div class="w-24 h-24 rounded-md mr-4 bg-gray-200 flex items-center justify-center text-5xl">${getEmojiForProduct(store.name)}</div><div class="flex-1"><h3 class="font-bold text-lg text-gray-900">${store.name}</h3><div class="flex items-center text-sm text-gray-600 mt-1"><i class="ph ph-star text-yellow-400 text-base mr-1"></i><span class="font-bold">${store.rating}</span><span class="mx-1">|</span><span>리뷰 ${store.reviews.length}</span></div><div class="mt-2">${tagsHtml}</div></div>`; container.appendChild(card); }); }
    function renderStoreDetail({ storeId, from, category }) { const store = storeData.find(s => s.id === storeId); if (!store) return; trackViewedStore(storeId); document.getElementById('storeDetailTitle').textContent = store.name; document.getElementById('backToCategoryBtn').onclick = () => showScreen(from, from === 'categoryListScreen' ? category : null); renderFavoriteButtons(storeId); document.getElementById('storeDetailInfo').innerHTML = `<div class="flex items-center"><div class="w-28 h-28 rounded-lg mr-4 bg-gray-200 flex items-center justify-center text-6xl">${getEmojiForProduct(store.name)}</div><div><h2 class="text-2xl font-bold">${store.name}</h2><div class="flex items-center text-lg text-gray-600 mt-2"><i class="ph ph-star-fill text-yellow-400 text-2xl mr-1"></i><span class="font-bold text-2xl">${store.rating}</span></div><p class="text-gray-500 mt-1">주문수 ${store.orders.toLocaleString()}</p></div></div>`; const productListContainer = document.getElementById('storeProductListContainer'); productListContainer.innerHTML = ''; if (store.products && store.products.length > 0) { store.products.forEach(product => { const productEl = document.createElement('div'); productEl.className = 'flex items-center justify-between bg-gray-50 p-4 rounded-lg'; productEl.innerHTML = `<div class="flex items-center"><div class="w-16 h-16 rounded-md mr-4 bg-gray-200 flex items-center justify-center text-3xl">${product.emoji}</div><div><h4 class="font-semibold text-gray-800">${product.name}</h4><p class="text-indigo-600 font-bold mt-1">${product.price.toLocaleString()}원</p></div></div><button class="add-to-cart-btn px-4 py-2 bg-indigo-600 text-white rounded-lg font-semibold hover:bg-indigo-700 transition-colors" data-store-id="${store.id}" data-store-name="${store.name}" data-product-id="${product.id}" data-product-name="${product.name}" data-product-price="${product.price}" data-product-emoji="${product.emoji}">담기</button>`; productListContainer.appendChild(productEl); }); } else { productListContainer.innerHTML = `<p class="text-center text-gray-500 py-4">등록된 상품 정보가 없습니다.</p>`; } const reviewsContainer = document.getElementById('storeReviewsContainer'); reviewsContainer.innerHTML = `<p class="text-center text-gray-500">아직 작성된 리뷰가 없습니다.</p>`; }
    function renderSearchScreen() { const searchInput = document.getElementById('searchInput'); searchInput.value = ''; renderSearchResults(storeData); }
    function navigateToStore(url) { if (url && url !== 'null' && url.trim() !== '') { const fullUrl = url.startsWith('http') ? url : `https://${url}`; window.open(fullUrl, '_blank'); } else { showMessage('길찾기 정보가 없는 상점입니다.'); } }
    function renderSearchResults(stores) { const container = document.getElementById('searchResultsContainer'); container.innerHTML = ''; if (!stores || stores.length === 0) { container.innerHTML = `<p class="text-center text-gray-500 mt-8">검색 결과가 없습니다.</p>`; return; } const uniqueStores = Array.from(new Map(stores.map(store => [store.id, store])).values()); uniqueStores.forEach(store => { const storeItem = document.createElement('div'); storeItem.className = 'search-store-item'; const productsList = store.products.slice(0, 5).map(p => p.name).join(', ') + (store.products.length > 5 ? '...' : ''); storeItem.innerHTML = `<div class="flex justify-between items-start mb-2"><h3 class="text-lg font-bold text-gray-900">${store.name}</h3><span class="bg-green-100 text-green-800 text-xs font-semibold px-2 py-1 rounded-full flex-shrink-0 ml-2">${store.category}</span></div><p class="text-sm text-gray-700 mb-2 truncate"><b>주요상품:</b> ${productsList || '정보 없음'}</p><p class="text-sm text-gray-500 mb-4">${store.description || ''}</p><div class="flex justify-end gap-2 mt-auto"><button class="navigate-btn px-4 py-2 text-sm font-semibold bg-blue-500 text-white rounded-full hover:bg-blue-600" data-url="${store.url}">길찾기</button><button class="store-link px-4 py-2 text-sm font-semibold bg-orange-500 text-white rounded-full hover:bg-orange-600" data-store-id="${store.id}" data-from="searchScreen">가게보기</button></div>`; container.appendChild(storeItem); }); }
    function handleSmartSearch() { const searchInput = document.getElementById('smartSearchInput'); const container = document.getElementById('smartSearchResultsContainer'); const searchTerms = searchInput.value.split(',').map(term => term.trim().split(' ')[0]).filter(term => term.length > 0); if (searchTerms.length === 0) { container.innerHTML = '<p class="text-center text-gray-500 py-10">찾고 싶은 상품을 쉼표(,)로 구분하여 검색해 보세요.<br>또는 저장된 레시피를 이용해 보세요.</p>'; return; } let resultsHtml = ''; searchTerms.forEach(term => { const matchingProducts = []; storeData.forEach(store => { store.products.forEach(product => { if (product.name.toLowerCase().includes(term.toLowerCase())) { matchingProducts.push({ ...product, storeId: store.id, storeName: store.name, rating: store.rating, reviews: store.reviews.length }); } }); }); matchingProducts.sort((a, b) => a.price - b.price); resultsHtml += `<div class="border rounded-lg mb-3 overflow-hidden"><button class="smart-accordion-toggle w-full p-4 text-left bg-gray-50 hover:bg-gray-100 flex justify-between items-center"><span class="font-bold text-lg text-indigo-700">'${term}' 검색 결과</span><i class="ph ph-caret-down text-xl transition-transform"></i></button><div class="smart-accordion-content hidden bg-white p-4 space-y-3">`; if (matchingProducts.length > 0) { matchingProducts.forEach(product => { resultsHtml += `<div class="flex items-center bg-white p-3 rounded-lg border"><div class="flex-1"><button class="font-semibold text-gray-800 hover:underline store-link" data-store-id="${product.storeId}" data-from="smartShoppingScreen">${product.storeName}</button><div class="flex items-center text-sm text-gray-500 mt-1"><i class="ph ph-star text-yellow-400 mr-1"></i> ${product.rating}<span class="mx-2">|</span><span class="font-bold text-indigo-600">${product.price.toLocaleString()}원</span></div></div><button class="add-to-cart-btn ml-4 px-4 py-2 bg-indigo-600 text-white text-sm font-semibold rounded-lg hover:bg-indigo-700 transition-colors whitespace-nowrap" data-store-id="${product.storeId}" data-store-name="${product.storeName}" data-product-id="${product.id}" data-product-name="${product.name}" data-product-price="${product.price}" data-product-emoji="${product.emoji}">담기</button></div>`; }); } else { resultsHtml += `<p class="text-center text-gray-500 py-4">'${term}'을(를) 판매하는 가게를 찾을 수 없습니다.</p>`; } resultsHtml += `</div></div>`; }); container.innerHTML = resultsHtml; }
    function renderFavoriteButtons(storeId) { const container = document.getElementById('favoriteButtonsContainer'); if (!container) return; const storeIdNum = parseInt(storeId, 10); const isFavorited = favoriteStores.includes(storeIdNum); if (isFavorited) { container.innerHTML = `<button class="remove-favorite-btn text-sm font-semibold text-gray-600 bg-gray-200 px-3 py-2 rounded-lg hover:bg-gray-300" data-store-id="${storeIdNum}">관심 가게 삭제</button>`; } else { container.innerHTML = `<button class="add-favorite-btn text-sm font-semibold text-indigo-600 bg-indigo-100 px-3 py-2 rounded-lg hover:bg-indigo-200" data-store-id="${storeIdNum}">관심 가게 등록</button>`; } }
    function toggleFavorite(storeId) { const storeIdNum = parseInt(storeId, 10); const index = favoriteStores.indexOf(storeIdNum); if (index > -1) { favoriteStores.splice(index, 1); } else { favoriteStores.push(storeIdNum); } if (document.getElementById('storeDetailScreen').classList.contains('active')) { renderFavoriteButtons(storeIdNum); } if (document.getElementById('favoritesScreen').classList.contains('active')) { renderFavoritesScreen(); } }
    function renderFavoritesScreen() { const container = document.getElementById('favoritesScreen').querySelector('main'); container.innerHTML = ''; const favoriteStoreDetails = storeData.filter(store => favoriteStores.includes(store.id)); if (favoriteStoreDetails.length === 0) { container.innerHTML = `<p class="text-center text-gray-500 p-10">관심 가게가 없습니다.</p>`; return; } favoriteStoreDetails.forEach(store => { const tagsHtml = store.mainTags.map(tag => `<span class="bg-gray-200 text-gray-700 text-xs font-medium mr-2 px-2 py-1 rounded">${tag}</span>`).join(''); const card = document.createElement('div'); card.className = 'flex items-start bg-white p-4 rounded-lg border border-gray-200 relative cursor-pointer store-link'; card.dataset.storeId = store.id; card.dataset.from = 'favoritesScreen'; card.innerHTML = `<div class="w-24 h-24 rounded-md mr-4 bg-gray-200 flex items-center justify-center text-5xl">${getEmojiForProduct(store.name)}</div><div class="flex-1"><h3 class="font-bold text-lg text-gray-900">${store.name}</h3><div class="flex items-center text-sm text-gray-600 mt-1"><i class="ph ph-star text-yellow-400 text-base mr-1"></i><span class="font-bold">${store.rating}</span><span class="mx-1">|</span><span>리뷰 ${store.reviews.length}</span></div><div class="mt-2">${tagsHtml}</div></div>`; container.appendChild(card); }); }
    function loadRecipesFromStorage() { const recipes = localStorage.getItem('cheongnyamri.recipes'); userRecipes = recipes ? JSON.parse(recipes) : []; }
    function saveRecipesToStorage() { localStorage.setItem('cheongnyamri.recipes', JSON.stringify(userRecipes)); }
    function renderMyRecipes() { const container = document.getElementById('myRecipesList'); container.innerHTML = ''; if (userRecipes.length === 0) { container.innerHTML = `<p class="text-center text-sm text-gray-500 py-2">저장된 레시피가 없습니다.</p>`; } else { userRecipes.forEach(recipe => { const recipeEl = document.createElement('div'); recipeEl.className = 'flex justify-between items-center bg-gray-100 p-3 rounded-lg'; recipeEl.innerHTML = `<button class="font-semibold text-left flex-1 load-recipe-btn" data-recipe-id="${recipe.id}">${recipe.name}</button><button class="edit-recipe-btn p-1 text-gray-500 hover:text-indigo-600" data-recipe-id="${recipe.id}"><i class="ph ph-pencil-simple"></i></button>`; container.appendChild(recipeEl); }); } }
    function showRecipeModal(recipeId = null) { showScreen('recipeModalScreen'); const titleEl = document.getElementById('recipeModalTitle'); const nameInput = document.getElementById('recipeNameInput'); const idInput = document.getElementById('recipeIdInput'); const ingredientsContainer = document.getElementById('recipeIngredientsContainer'); const deleteBtn = document.getElementById('deleteRecipeBtn'); ingredientsContainer.innerHTML = ''; if (recipeId) { const recipe = userRecipes.find(r => r.id === recipeId); titleEl.textContent = '레시피 수정'; nameInput.value = recipe.name; idInput.value = recipe.id; recipe.ingredients.forEach(ing => addIngredientField(ing.name, ing.request)); deleteBtn.classList.remove('hidden'); } else { titleEl.textContent = '새 레시피 추가'; nameInput.value = ''; idInput.value = ''; addIngredientField(); deleteBtn.classList.add('hidden'); } }
    function addIngredientField(name = '', request = '') { const container = document.getElementById('recipeIngredientsContainer'); const div = document.createElement('div'); div.className = 'flex items-center space-x-2 bg-white p-2 border rounded-lg'; div.innerHTML = `<div class="flex-1"><input type="text" class="recipe-ingredient-name w-full text-sm p-1 border-b" placeholder="재료 (예: 김치 1포기)" value="${name}"><input type="text" class="recipe-ingredient-request w-full text-xs p-1 text-gray-500" placeholder="요청사항 (예: 찌개용으로 썰어주세요)" value="${request}"></div><button class="remove-ingredient-btn text-red-500 p-1"><i class="ph ph-trash"></i></button>`; container.appendChild(div); }
    function saveRecipe() { const recipeId = document.getElementById('recipeIdInput').value; const recipeName = document.getElementById('recipeNameInput').value.trim(); if (!recipeName) { showMessage('레시피 이름을 입력해주세요.'); return; } const ingredients = []; document.querySelectorAll('#recipeIngredientsContainer > div').forEach(div => { const name = div.querySelector('.recipe-ingredient-name').value.trim(); const request = div.querySelector('.recipe-ingredient-request').value.trim(); if (name) { ingredients.push({ name, request }); } }); if (ingredients.length === 0) { showMessage('하나 이상의 재료를 입력해주세요.'); return; } if (recipeId) { const recipeIndex = userRecipes.findIndex(r => r.id === recipeId); userRecipes[recipeIndex] = { ...userRecipes[recipeIndex], name: recipeName, ingredients }; } else { const newRecipe = { id: `recipe-${Date.now()}`, name: recipeName, ingredients: ingredients }; userRecipes.push(newRecipe); } saveRecipesToStorage(); showScreen('smartShoppingScreen'); }
    function deleteRecipe() { const recipeId = document.getElementById('recipeIdInput').value; userRecipes = userRecipes.filter(r => r.id !== recipeId); saveRecipesToStorage(); showScreen('smartShoppingScreen'); }
    function loadRecentlyViewedFromStorage() { const recent = localStorage.getItem('cheongnyamri.recentStores'); recentlyViewedStores = recent ? JSON.parse(recent) : []; }
    function saveRecentlyViewedToStorage() { localStorage.setItem('cheongnyamri.recentStores', JSON.stringify(recentlyViewedStores)); }
    function trackViewedStore(storeId) { recentlyViewedStores = recentlyViewedStores.filter(item => item.storeId !== storeId); recentlyViewedStores.unshift({ storeId: storeId, viewedAt: new Date().toISOString() }); recentlyViewedStores = recentlyViewedStores.slice(0, 20); saveRecentlyViewedToStorage(); }
    ...
    // ==================== 픽업 및 주문 내역 기능 ====================
    async function handleCheckout() { const userId = document.getElementById('userIdInput').value.trim(); if (!userId) { showMessage('사용자 ID를 입력해주세요.'); return; } const orderId = `CNY-${Date.now()}`; const newOrder = { orderId: orderId, userId: userId, cart: JSON.parse(JSON.stringify(shoppingCart)), date: new Date().toLocaleDateString('ko-KR', { year: 'numeric', month: 'long', day: 'numeric', weekday: 'short' }), totalPrice: Object.values(shoppingCart).reduce((sum, store) => sum + Object.values(store.items).reduce((s, item) => s + item.price * item.quantity, 0), 0) }; try { const response = await fetch(`${API_BASE_URL}/api/orders`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(newOrder) }); if (!response.ok) throw new Error('서버에 주문을 저장하지 못했습니다.'); completedOrders.push(newOrder); for (const storeId in newOrder.cart) { const store = newOrder.cart[storeId]; const firstItem = Object.values(store.items)[0]; const totalItems = Object.keys(store.items).length; const message = `'${store.storeName}'에서 ${firstItem.name}` + (totalItems > 1 ? ` 외 ${totalItems - 1}건` : '') + ` 구매가 완료되었습니다.`; const newNotification = { id: Date.now() + parseInt(storeId), message: message, read: false }; notifications.unshift(newNotification); showToast(message); } saveNotificationsToStorage(); updateNotificationIndicator(); shoppingCart = {}; updateCartCountIndicator(); const modal = document.querySelector('.modal-overlay'); if (modal) modal.remove(); showScreen('pickupScreen', { userId: userId }); } catch (error) { console.error('Checkout Error:', error); showMessage('주문 처리 중 오류가 발생했습니다.'); } }
    ...
    async function renderFilteredPickupOrders(userId) { const resultsContainer = document.getElementById('pickupOrderResultsContainer'); resultsContainer.innerHTML = `<p class="text-center text-gray-500">주문 내역을 불러오는 중...</p>`; try { const response = await fetch(`${API_BASE_URL}/api/orders/${userId}`); if (!response.ok) throw new Error('주문 내역을 불러오지 못했습니다.'); const userOrders = await response.json(); if (userOrders.length === 0) { resultsContainer.innerHTML = `<p class="text-center text-gray-500 p-10">해당 ID의 주문 내역이 없습니다.</p>`; return; } resultsContainer.innerHTML = ''; userOrders.forEach(order => { const orderEl = document.createElement('div'); orderEl.className = 'mb-8'; const details = order.order_details; let itemsHtml = ''; for (const storeId in details.cart) { const store = details.cart[storeId]; const items = Object.values(store.items).map(item => `${item.name} ${item.quantity}개${item.request ? ` (${item.request})` : ''}`).join(', '); itemsHtml += `<div class="flex items-start bg-gray-100 p-3 rounded-lg"><div class="w-12 h-12 rounded-md mr-4 bg-gray-200 flex items-center justify-center text-3xl">${getEmojiForProduct(store.storeName)}</div><div><p class="font-semibold text-gray-800">${store.storeName}</p><p class="text-gray-600">${items}</p></div></div>`; } const qrCodeData = JSON.stringify({ userId: order.user_id, orderId: order.order_id }); orderEl.innerHTML = `<div class="bg-gray-50 p-6 rounded-lg text-center"><h2 class="text-lg font-semibold text-gray-800">픽업 준비 완료</h2><p class="text-gray-500 mt-1">픽업 센터에서 아래 QR코드를 보여주세요.</p><div id="qrcode-${order.order_id}" class="mt-4 flex justify-center"></div><p class="mt-4 text-sm font-mono bg-gray-200 p-2 rounded inline-block">주문번호: ${order.order_id}</p><p class="mt-2 text-sm font-mono bg-gray-200 p-2 rounded inline-block">사용자 ID: ${order.user_id}</p></div><div class="mt-8"><h3 class="text-lg font-bold text-gray-800 mb-4">주문 내역</h3><div class="space-y-3">${itemsHtml}</div></div><hr class="my-8 border-gray-300">`; resultsContainer.appendChild(orderEl); new QRCode(document.getElementById(`qrcode-${order.order_id}`), { text: qrCodeData, width: 180, height: 180 }); }); } catch (error) { console.error('Fetch Orders Error:', error); resultsContainer.innerHTML = `<p class="text-center text-red-500 p-10">오류가 발생했습니다.</p>`; } }
    async function saveOrder(userId, items) {
    ...
    function setupAllEventListeners() {
        document.getElementById('searchInput').addEventListener('input', (e) => {
            ...
            const filteredStores = storeData.filter(store => 
                (store.name.toLowerCase() + store.products.map(p => p.name).join('')).includes(query)
            );
        ...
        document.getElementById('mealKitSearchInput').addEventListener('input', (e) => renderMealKitList(e.target.value));
        document.querySelectorAll('.category-sort-btn').forEach(button => { button.addEventListener('click', (e) => { 
        ...
        document.getElementById('recipeIngredientsContainer').addEventListener('click', e => { if (e.target.closest('.remove-ingredient-btn')) e.target.closest('.flex').remove(); });
        
        document.body.addEventListener('click', (e) => {
            const targetMap = {
                '.meal-kit-item': (target) => showMealKitDetailModal(target.dataset.kitId),
                '.add-kit-to-recipe-btn': (target) => addMealKitToMyRecipes(target.dataset.kitId),
                '.add-to-cart-btn': (target) => showAddToCartModal(target.dataset),
                '.add-favorite-btn': (target) => toggleFavorite(target.dataset.storeId),
                '.remove-favorite-btn': (target) => toggleFavorite(target.dataset.storeId),
                '.store-link': (target) => showScreen('storeDetailScreen', { storeId: parseInt(target.dataset.storeId), from: target.dataset.from, category: target.dataset.category }),
                '.navigate-btn': (target) => navigateToStore(target.dataset.url),
                '.smart-accordion-toggle': (target) => { const content = target.nextElementSibling; const icon = target.querySelector('i'); content.classList.toggle('hidden'); icon.classList.toggle('rotate-180'); },
                '.load-recipe-btn': (target) => { const recipe = userRecipes.find(r => r.id === target.dataset.recipeId); document.getElementById('smartSearchInput').value = recipe.ingredients.map(i => i.name).join(', '); handleSmartSearch(); },
                '.edit-recipe-btn': (target) => showRecipeModal(target.dataset.recipeId)
            };
            for (const selector in targetMap) {
                const target = e.target.closest(selector);
                if (target) {
                    targetMap[selector](target);
                    return;
                }
            }
        });

        document.querySelectorAll('.nav-btn').forEach(button => { button.addEventListener('click', (e) => { const screenId = e.currentTarget.dataset.screen; if (screenId) showScreen(screenId); }); });
        const frame = document.getElementById('marketMapFrame');
        frame.addEventListener('load', () => { document.getElementById('mapLoading').style.display = 'none'; frame.style.display = 'block'; });
    }
